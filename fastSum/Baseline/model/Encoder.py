...
class Encoder(nn.Module):
    def __init__(self, hps, embed):
        ...
        self.position_embedding = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(sent_max_len + 1, embed_size, padding_idx=0),
            freeze=True,
        )
    ...
    def calculate_fan_in_and_fan_out(tensor):
        ...
        if dimensions < 2:
            print(
                "[Error] Fan in and fan out can not be computed for tensor with less than 2 dimensions"
            )
            raise ValueError(
                "[Error] Fan in and fan out can not be computed for tensor with less than 2 dimensions"
            )
    ...
    def forward(self, input):
        ...
        input_sent_len = ((input != 0).sum(dim=1)).int().cpu()  # [batch_size*N, 1]
        enc_embed_input = self.embed(input)  # [batch_size*N, L, D]

        input_pos = torch.Tensor(
            [
                np.hstack(
                    (np.arange(1, sentlen + 1), np.zeros(self.sent_max_len - sentlen))
                )
        ...
        ]  # kernel_sizes * (batch*N, Co, W)
        enc_maxpool_output = [
            F.max_pool1d(x, x.size(2)).squeeze(2) for x in enc_conv_output
        ]  # kernel_sizes * (batch*N, Co)
        sent_embedding = torch.cat(
            enc_maxpool_output, 1
        )  # (batch*N, Co * kernel_sizes)
...
class DomainEncoder(Encoder):
    ...
    def forward(self, input, domain):
        ...
        enc_domain_input = enc_domain_input.unsqueeze(1).expand(
            batch_size, N, -1
        )  # [batch, N, D]
...
class MultiDomainEncoder(Encoder):
    ...
    def forward(self, input, domain):
        ...
        sent_embedding = super().forward(input)
        domain_padding = (
            torch.arange(self.domain_size).unsqueeze(0).expand(batch_size, -1)
        )
        domain_padding = (
            domain_padding.cuda().view(-1)
            if self._hps.cuda
            else domain_padding.view(-1)
        )  # [batch * domain_size]

        enc_domain_input = self.domain_embedding(
            domain_padding
        )  # [batch * domain_size, D]
        enc_domain_input = (
            enc_domain_input.view(batch_size, self.domain_size, -1)
            * domain.unsqueeze(-1).float()
        )  # [batch, domain_size, D]

        # logger.info(enc_domain_input[:5,:])   # [batch, domain_size, D]

        enc_domain_input = enc_domain_input.sum(1) / domain.sum(1).float().unsqueeze(
            -1
        )  # [batch, D]
        enc_domain_input = enc_domain_input.unsqueeze(1).expand(
            batch_size, N, -1
...
class BertEncoder(nn.Module):
    def __init__(self, hps):
        ...
        # word embedding
        self._bert = BertModel.from_pretrained("../Load_Files/uncased_L-24_H-1024_A-16")
        self._bert.eval()
    ...
    def pad_encoder_input(self, input_list):
        ...
        for i in range(len(input_list)):
            ...
            if seq_len > max_len:
                ...
                pad_tensor = (
                    torch.zeros(max_len - seq_len, hidden_size).cuda()
                    if self._cuda
                    else torch.zeros(max_len - seq_len, hidden_size)
                )
    ...
    def forward(self, inputs, input_masks, enc_sent_len):
        ...
        for i in range(batch_size):
            ...
            out = torch.cat(out[-4:], dim=-1).squeeze(
                0
            )  # [doc_len=512, hidden_state=4096]
            ...
            for length in sent_len:
                if length != 0 and last_end < 511:
                    enc_sent_input.append(
                        out[last_end : min(511, last_end + length), :]
                    )
                    last_end += length
                else:
                    pad_tensor = (
                        torch.zeros(self.sent_max_len, hidden_size).cuda()
                        if self._hps.cuda
                        else torch.zeros(self.sent_max_len, hidden_size)
                    )
                    enc_sent_input.append(pad_tensor)

            # pad the sentence
            enc_sent_input_pad = self.pad_encoder_input(
                enc_sent_input
            )  # [N, seq_len, hidden_state=4096]
...
class ELMoEndoer(nn.Module):
    def __init__(self, hps):
        ...
        elmo_dim = 1024
        options_file = (
            "../Load_Files/elmo_2x4096_512_2048cnn_2xhighway_5.5B_options.json"
        )
        weight_file = (
            "../Load_Files/elmo_2x4096_512_2048cnn_2xhighway_5.5B_weights.hdf5"
        )

        # elmo_dim = 512
        # options_file = "../Load_Files/elmo_2x2048_256_2048cnn_1xhighway_options.json"
        # weight_file = "../Load_Files/elmo_2x2048_256_2048cnn_1xhighway_weights.hdf5"
    ...
    def forward(self, input):
        ...
        input = input.view(
            batch_size * N, seq_len, -1
        )  # [batch_size*N, seq_len, character_len]
        input_sent_len = ((input.sum(-1) != 0).sum(dim=1)).int()  # [batch_size*N, 1]
        # logger.debug(input_sent_len.view(batch_size, -1))
        enc_embed_input = self.elmo(input)["elmo_representations"][
            0
        ]  # [batch_size*N, L, D]
...
class ELMoEndoer2(nn.Module):
    ...
    def forward(self, inputs, input_masks, enc_sent_len):
        ...
        elmo_output = self.elmo(inputs)["elmo_representations"][
            0
        ]  # [batch_size, 512, D]
        ...
        for i in range(batch_size):
            ...
            for length in sent_len:
                if length != 0 and last_end < 512:
                    enc_sent_input.append(
                        out[last_end : min(512, last_end + length), :]
                    )
                    last_end += length
                else:
                    pad_tensor = (
            ...
            input_pad_list.append(
                torch.stack(enc_sent_input_pad)
            )  # batch * [N, max_len, hidden_state]

...
class TransformerModel(nn.Module):
    def __init__(self, hps, embed):
        ...
        self.sent_embedding_size = (
            hps.max_kernel_size - hps.min_kernel_size + 1
        ) * hps.output_channel
        ...
        self.sent_pos_embed = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(
                hps.doc_max_timesteps + 1, self.hidden_size, padding_idx=0
            ),
            freeze=True,
        )
    ...
    def forward(self, words, seq_len):
        ...
        self.slf_attn_mask = self.slf_attn_mask.unsqueeze(1).expand(
            -1, N, -1
        )  # [batch, N, N]
        self.non_pad_mask = input_len.unsqueeze(-1)  # [batch, N, 1]

        input_doc_len = input_len.sum(dim=1).int().cpu()  # [batch]
        sent_pos = torch.Tensor(
            [
                np.hstack((np.arange(1, doclen + 1), np.zeros(N - doclen)))
                for doclen in input_doc_len
            ]
        )
        ...
        for enc_layer in self.layer_stack:
            ...
            enc_input, enc_slf_atten = enc_layer(
                enc_input,
        ...
        self.dec_output_state = torch.cat(
            enc_input_list[-4:]
        )  # [4, batch_size, N, hidden_state]

...
def setup_training(model, train_loader, valid_loader, testloader, hps):
    ...
    run_training(
        model, train_loader, valid_loader, testloader, hps
    )  # this is an infinite loop until interrupted


def run_training(model, train_loader, valid_loader, testloader, hps):
    ...
    criterion = MyCrossEntropyLoss(
        pred="p_sent", target=Const.TARGET, mask=Const.INPUT_LEN, reduce="none"
    )
    ...
    trainer = Trainer(
        ...
        validate_every=-1,
        save_path=eval_dir,
        callbacks=[TrainCallback(hps, patience=10)],
        use_tqdm=False,
    )
    ...
    logger.info(
        "[INFO] best eval model in epoch %d and iter %d",
        train_info["best_epoch"],
        train_info["best_step"],
    )
...
def run_test(model, loader, hps):
    ...
    logger.info(
        "[INFO] Restoring %s for testing...The path is %s",
        hps.test_model,
        bestmodel_load_path,
    )
    ...
    if hps.use_pyrouge:
        ...
        tester = Tester(
            ...
            metrics=[
                LabelFMetric(pred="prediction"),
                PyRougeMetric(hps, pred="prediction"),
            ],
...
def main():
    ...
    parser.add_argument(
        "--train_data_path",
        type=str,
        default="./testdata/train.jsonl",
        help="Path expression to pickle train datafiles.",
    )
    parser.add_argument(
        "--test_data_path",
        type=str,
        default="./testdata/test.jsonl",
        help="Path expression to pickle test datafiles.",
    )
    parser.add_argument(
        "--valid_data_path",
        type=str,
        default="./testdata/val.jsonl",
        help="Path expression to pickle valid datafiles.",
    )
    parser.add_argument(
        "--vocab_path",
        type=str,
        default="./testdata/vocab",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Size of vocabulary. These will be read from the vocabulary file in order. If the vocabulary file contains fewer words than this number, or if this number is set to 0, will take all words in the vocabulary file.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="choose different model to test [evalbestmodel/evalbestFmodel/trainbestmodel/trainbestFmodel/earlystop]",
    )
    parser.add_argument(
        "--use_pyrouge", action="store_true", default=False, help="use_pyrouge"
    )
    ...
    TEST_DATA_FILE = args.test_data_path
    ...
    if hps.mode == "test":
        ...
        logger.info(hps)
        paths = {"train": TRAIN_DATA_FILE, "test": TEST_DATA_FILE}
        ...
        print(paths)
        db = dbPipe.process_from_file(paths)
        print(db.get_dataset_names())

    print("######## Dataset Ending ########")
    ...
    if args.embedding == "glove":
        ...
        if hps.word_embedding:
            ...
            pretrained_weight = embed_loader.load_with_vocab(
                hps.embedding_path, vocab
            )  # unfound with random init
    ...
    if hps.mode == "train":
        ...
        db.get_dataset("test").set_target("text", "summary")
        run_test(model, db.get_dataset("test"), hps)
    else:
